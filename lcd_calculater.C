...
/* Author : Prasanna */
/* Date   : 05-11-2022 */
/* Filename:using_calculater_lcd_blink */
/* Description :calculater_lcd_blink*/
/***************************************/
#include<htc.h>
#include<stdio.h>
...
{  
   ...
   PORTC=0x00;
    ...
    
    {
		...
		row_3 =1;//which row is push button 
		row_2 =0;
		row_1 =0;
		row_4 =0;
		
		if (PORTD == 0x14)//in calculater which push button we need 
		  
		{
			lcd_cmd(0x01);//clear display
			lcd_cmd(0x80);//in lcd where we need to print
			lcd_cmd(col_3 == 1);//which collam in push button
			lcd_data('1');//what we need to print in lcd
			__delay_ms(100);
		}
		row_4 =1;
		row_2 =0;
		...
		//IN ABOVE THERE IS A ADD,SUB,MULT..SO WHAT WE NEED THAT ONE UNCOMMAND
